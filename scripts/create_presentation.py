...
def build_deck():
    ...
    slide = add_slide(prs, INK)
    title(slide, "On-premise-ready by design", "Architecture and technology stack keep evidence, models, and decisions governable", 4, True)
    add_text(slide, "ARCHITECTURE FLOW", 0.82, 1.93, 2.1, 0.22, 9, MINT, True)
    # Explicit left-to-right flow: client -> API boundary -> local intelligence -> stores.
    boxes = [("Portal", "Streamlit\nApplicant + admin", 0.82, CYAN), ("API boundary", "FastAPI\nJWT + OpenAPI", 3.0, MINT), ("Intelligence", "Ingestion / OCR\nAgents + tools", 5.18, AMBER), ("Evidence stores", "SQLite / Postgres\nChroma + raw files", 7.36, CORAL)]
    for label, detail, x, accent in boxes:
        rect(slide, x, 2.35, 1.82, 1.35, RGBColor(42, 61, 66), RGBColor(72, 95, 97), True)
        rect(slide, x, 2.35, 1.82, 0.1, accent, accent, True)
        add_text(slide, label, x + 0.16, 2.63, 1.5, 0.25, 13, PAPER, True, DISPLAY)
        add_text(slide, detail, x + 0.16, 3.04, 1.5, 0.45, 10, RGBColor(190, 211, 205))
    for x in [2.64, 4.82, 7.0]:
        line(slide, x, 3.02, x + 0.35, 3.02, MINT, 2)
    rect(slide, 9.54, 2.35, 2.84, 1.35, RGBColor(42, 61, 66), RGBColor(72, 95, 97), True)
    add_text(slide, "Controlled adapters", 9.78, 2.63, 2.3, 0.25, 13, PAPER, True, DISPLAY)
    add_text(slide, "Ollama local default\nBedrock / Anthropic / OpenAI\nOptional Exa name-only check", 9.78, 3.04, 2.25, 0.48, 10, RGBColor(190, 211, 205))
    line(slide, 9.18, 3.02, 9.48, 3.02, CORAL, 2)
    add_text(slide, "TECHNOLOGY STACK", 0.82, 4.18, 2.1, 0.22, 9, AMBER, True)
    stack = [("UI", "Streamlit", CYAN), ("API", "FastAPI + Uvicorn", MINT), ("AI", "Strands Agents + Pydantic", AMBER), ("Documents", "PyMuPDF / python-docx / OCR", CORAL), ("Search", "ChromaDB + embeddings", CYAN), ("Data", "SQLite / PostgreSQL", MINT_DARK), ("Ops", "Pytest + Ruff + OpenTelemetry", AMBER)]
    for i, (label, value, accent) in enumerate(stack):
        col = i % 4
        row = i // 4
        x = 0.82 + col * 3.02
        y = 4.58 + row * 0.7
        rect(slide, x, y, 2.72, 0.48, RGBColor(42, 61, 66), RGBColor(72, 95, 97), True)
        add_text(slide, label, x + 0.14, y + 0.13, 0.68, 0.17, 9, accent, True)
        add_text(slide, value, x + 0.85, y + 0.11, 1.72, 0.2, 10, PAPER, True)
    add_text(slide, "Restricted evidence remains local by default; provider and database choices are configuration, not business logic.", 0.82, 6.55, 11.7, 0.25, 12, RGBColor(192, 212, 206), italic=True)
    footer(slide, True)

    # 5. Agent system
    slide = add_slide(prs)
    title(slide, "Bounded intelligence", "Agents are configured capabilities, not autonomous decision-makers", 5)
    ...
    # 6. Explainability
    slide = add_slide(prs)
    title(slide, "Evidence before arithmetic", "Validation and scoring expose uncertainty instead of hiding it", 6)
    ...
    # 7. Human workflow
    slide = add_slide(prs, INK)
    title(slide, "Human control is a product feature", "The workflow makes intervention, override, and accountability explicit", 7, True)
    ...
    # 8. Dataset
    slide = add_slide(prs)
    title(slide, "Built to be challenged", "A synthetic benchmark exercises the failure modes that matter", 8)
    ...
    # 9. Demo
    slide = add_slide(prs)
    title(slide, "Reviewer demo", "A five-minute path through the working POC", 9)
    ...
    add_text(slide, "WHAT WE BUILT", 0.82, 0.78, 2.2, 0.25, 10, INK, True)
    add_text(slide, "A reviewer-facing\napplication intelligence\nplatform", 0.82, 1.42, 5.25, 1.45, 31, INK, True, DISPLAY)
    add_text(slide, "Submission, extraction, validation, duplicate checks, explainable scoring, routing, feedback, approvals, audit, and final human review.", 0.86, 3.25, 5.25, 0.72, 15, INK_2)
    rect(slide, 6.55, 0.78, 5.98, 4.78, INK, INK, True)
    add_text(slide, "HOW IT PROVES ANSWERS ARE GROUNDED", 6.92, 1.16, 5.1, 0.25, 10, MINT, True)
    proof = [("01", "Source-linked", "Raw files and extracted document manifest remain available."), ("02", "Structured", "Pydantic results separate fields, flags, score, and uncertainty."), ("03", "Traceable", "Analysis events capture stages, tool calls, reasoning, and retries."), ("04", "Testable", "12 labeled synthetic cases verify extraction, conflicts, duplicates, risk, and score arithmetic.")]
    for i, (num, label, detail) in enumerate(proof):
        y = 1.7 + i * 0.82
        circle(slide, 6.92, y, 0.42, [CYAN, MINT, AMBER, CORAL][i])
        add_text(slide, num, 6.92, y + 0.11, 0.42, 0.16, 8.5, INK, True, FONT, PP_ALIGN.CENTER)
        add_text(slide, label, 7.52, y - 0.01, 1.42, 0.22, 12, PAPER, True)
        add_text(slide, detail, 9.05, y - 0.01, 3.08, 0.42, 10.5, RGBColor(190, 211, 205))
    rect(slide, 0.82, 5.95, 11.7, 0.64, RGBColor(230, 246, 242), RGBColor(230, 246, 242), True)
    add_text(slide, "Grounding rule", 1.1, 6.16, 1.4, 0.2, 11, MINT_DARK, True)
    add_text(slide, "No evidence, no confident claim. No human review record, no final approval or rejection.", 2.7, 6.14, 8.9, 0.22, 13, INK, True)
